...
options {
 ...
 field_decl:        
              ( TYPE ID (VIRGULA TYPE ID )?  PONT_V |  TYPE ID  CL INT_LITERAL CR (VIRGULA ID CL INT_LITERAL CR )? PONT_V );
                 
 
 method_decl:
              (TYPE | VOID)  ID PL (TYPE ID ( VIRGULA TYPE ID)*)? PR block ; 
 ...
 var_decl:
            TYPE ID (VIRGULA ID)* PONT_V ; 
	
 
 statement:
             location (IGUAL_OP|ASSING_OP ) expr PONT_V
 ...
 method_call: 
               method_name  PL   ( expr (VIRGULA expr)* )? PR 
